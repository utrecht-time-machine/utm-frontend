...
export class OrganisationService {
  ...
  constructor(private http: HttpClient, private utmTranslate: UtmTranslateService) {
    void this._loadAll();
  }
  ...
  public getByIds(organisationIds: string[]): Organisation[] | undefined {
    ...
    const filteredOrganisations = allOrganisations.filter(org => organisationIds.includes(org.id));
  ...
  public async getOrganisations(): Promise<Organisation[]> {
    const organisations: Organisation[] = await lastValueFrom(
      this.http.get<Organisation[]>(environment.apiUrl + environment.apiSuffixes.organisations),
    );

    UtilService.addUrlPrefixes(organisations, 'logo');
    this.utmTranslate.translateObjectsByKeys(organisations, environment.translateKeys.organisation);
